...
foreach ($visionFS->xml->shop->offers->offer as $item) {
    ...
    //sizes
    if (isset($item['param'])) {
        $sizes = is_array($item['param']) ?
            implode(';', array_unique($item['param'])) :
            $item['param'];
    } else {
        $sizes = '';
    }
...
foreach ($itemsFromDB as $url => $item) {
    ...
    if (!key_exists($url, $xmlOffersURLs)) {
        ...
        //update price
        $retailPrice = ceil($xmlOffersURLs[$url]['price']);
        $salePrice   = ceil($xmlOffersURLs[$url]['price']/1.3);
