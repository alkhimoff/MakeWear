...
if ($_SESSION['status']=="admin"){
	...
	while($row = mysql_fetch_assoc($res)){
		...
		$sql2="
			...
			while($row2=mysql_fetch_assoc($res2))
			{
				...
				$pricee = $row2["commodity_price"];
				$price_opt = $row2["commodity_price2"];
				$priceee=$row2["price"];
				
				/*if($priceee==$pricee || $priceee!=$price_opt){
					$priceee=$pricee;	
					$price = $priceee/$count;
					$com_sum = $priceee;			
				}			
				...
				$client_sum["{$user_id}"] += $com_sum;
				// $client_sum["{$user_id}"] += $comission[$user_id];
				// $client_sum["{$user_id}"] += $delivery_price[$user_id];
	...
	foreach ($client_count as $key => $value) {
		$sql5 = "SELECT * 
				...
				WHERE  a.`user_id` ={$key} AND a.`status` < 7 ";
