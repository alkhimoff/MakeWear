...
if ($_SESSION['status']=="admin"){
	...
	WHERE  `com_status` =4 AND `status`=3";
	$res = mysql_query($sql);
	while($row = mysql_fetch_assoc($res)){
		...
		<td>
			<select size='1' name='status' id = 'select_status_com' rel = '{$row['id']}'>
	...
	$center = "
	<h1>Итого<br></h1>
	<h2>Сумма: {$result_sum};  Заказов: {$numssd};  Единиц: {$result_quantity};  К оплате: {$result_sum_to_pay};  Выручка: {$result_profit};</h2>
...
/*	foreach ($group_line as $key => $value) {
		...
		$group_line[$key].="<tr class = 'c2_group_line' id='{$row0['group_id']}' rel='sup_group' rel2='group_id'>
			...
			<select size='1' name='status' id = 'select_group_status' rel='{$key}'>
    				...
    				<option value='4'  {$selected_group4[$key]}>Оплачен</option>
    		</select></td>
			</tr>
			";
			$group_line[$key].="
