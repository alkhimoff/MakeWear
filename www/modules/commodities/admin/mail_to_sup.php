...
if ($_SESSION['status']=="admin")
{
	if(isset($_GET["id"]))
	{
		...
		foreach($rrrr as $key=>$value){
			$query = "UPDATE `sup_group` SET `sended` = 1 WHERE `group_id`={$group_id}";
			mysql_query($query);
			...
			WHERE `group_id` = {$group_id} AND `count`>0";
		...
		<p>Добрый день, пожалуйста подтвердите наличие данных моделей</p>
		...
		send_mime_mail_html($glb["dom_mail"],$glb["sys_mail"],$sup_name, "info@makewear.com.ua","utf-8","utf-8","Заказ от сайта {$glb["dom_mail"]}",$mail);
