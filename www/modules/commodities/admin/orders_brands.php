...
if ($_SESSION['status']=="admin")
{
	...
	if($commodity){	
		...
		foreach($commodity as $k_comid=>$v_catid){
			// echo $k_comid."=".$v_catid."<br/>";
			$groupid=$group_cat[$k_comid];

			$add_comm[$group_cat[$k_comid]]="";
			$add_comm2[$group_cat[$k_comid]]="";

			if($count1[$v_catid]=="" || $count1[$v_catid]==0)
				$count1[$v_catid]=0;	
			if($count2[$v_catid]=="" || $count2[$v_catid]==0)
				$count2[$v_catid]=0;			
			
			$shop_com=mysql_query("SELECT *, b.`id` AS iiid FROM `shop_commodity` AS a 
										INNER JOIN `shop_orders_coms` AS b ON a.`commodity_ID`=b.`com_id`
										INNER JOIN `shop_orders` AS c ON c.`id`=b.`offer_id`
										WHERE `com_status` IN (0,1,2,3) AND `commodity_ID`='{$k_comid}' ");
			...
			if($group_cat[$k_comid]!=0){
					$add_comm[$v_catid]="
							...
							";
					$add_group_cat[$v_catid].=$add_comm[$v_catid]; // Gorup		
				}
				...
				$add_group_cat2[$v_catid].=$add_comm2[$v_catid];
				}		
				$ret[$v_catid]="<tr><td colspan=11 >".$add_group_cat2[$v_catid]."</table></td></tr>";
		}
		
	}
	//var_dump($add_group_cat2[$group_cat[$k_comid]]);
...
//-------------Object group--------------------------------------------
		...
		$group_id_name2[$vk].=$add_group_cat[$ok]."</table></td></tr>";
	...
	if($ret){
		...
		foreach($ret as $key=>$val){
			if($count2[$key]==0){
				...
				$order22[$key]=$order2[$key];
			if($order1[$key]=="")
				$order1[$key]=1;
					
			if($order1[$key] && $order22[$key])
				$rating[$key]=$order1[$key]/$order22[$key];
			
			$rett[$key]="<tr class='tab_b' >
									<td>
										<h1 id='bre_name{$key}'>{$cat_name[$key]}</h1>
										<span class='cl_delll cll{$key}'>Удалить <img src='/templates/admin/img/btnbar_del.png'></span>
										<span class='cl_edittt cll{$key}'> Группировать<img src='/templates/admin/img/btnbar_edit.png'></span>
										<span class='obj_edittt{$key} obj_but' rel='{$key}' style='display:none; cursor:pointer'> Объединить группи<img src='/templates/admin/img/btnbar_edit.png'></span>	
									</td>
									<td align='center' class='up_site{$key}'></td>
									<td align='center' class='up_contact{$key}'></td>
									<td align='center' class='up_payment{$key}'></td>
									<td align='center' class='up_delivery{$key}'></td>
									<td align='center'>
										ЗАКАЗОВ<br>
										{$order1[$key]}									
									</td>
									...
									<td align='center'><div class='rat_body' ><span class='rating' >{$rating[$key]}</span></div></td>
									<td align='center'><span class='edit_brenda' rel='{$key}' style='cursor: pointer;' ><img src='/templates/admin/img/btnbar_edit.png'></span></td>
								</tr>";
			$center.=$rett[$key];
			$center.=$ret[$key];
		}
