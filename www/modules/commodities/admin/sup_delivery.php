...
if ($_SESSION['status']=="admin"){
	...
	";
	$sql0 ="SELECT * FROM `sup_group` WHERE `status` IN (4) ORDER BY `group_id` DESC"; 
	...
	while($r=mysql_fetch_assoc($res0)){
		...
		WHERE `group_id` = {$group_id} AND `com_status`=4";
	...
	//	}
		$group_head["{$group_id}"] = "<tr class ='c2_group_line' id='{$r['group_id']}' rel='sup_group' rel2='group_id'>
		<td></td>
		<td>№{$group_id}</td>
		<td>{$sup_name}</td>
		<td>{$total_count}</td>
		<td id ='del_price' class = 'cl_edit'>{$r['del_price']}</td>
		<td id ='weight' class = 'cl_edit'>{$r['weight']}</td>
		<td id ='ttn' class = 'cl_edit'>{$r['ttn']}</td>
		<td id ='del_comment' class = 'cl_edit'>{$r['del_comment']}</td>
		<td id ='from' class = 'cl_edit'>{$r['from']}</td>
		<td id ='to' class = 'cl_edit'>{$r['to']}</td>
		<td id ='contact' class = 'cl_edit'>{$r['contact']}</td>
		<td><select size='1' name='status' id = 'select_group_status' rel='{$group_id}'>
    				...
    				<option value='6' rel='{$offer_id}' {$selected_group6}>Отправлен</option>		
        		</select></td>
		</tr>
